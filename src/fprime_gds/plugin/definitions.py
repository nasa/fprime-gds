...
def gds_plugin(plugin_class):
    ...
    """
    assert isinstance(plugin_class, type), "Must supply @gds_plugin valid plugin classes"
    plugin_name = plugin_class.__name__
    spec_attr = f"{ PROJECT_NAME }_spec"
    spec_methods = inspect.getmembers(
        plugin_class,
        predicate=lambda item: hasattr(item, spec_attr)
    )
    ...
    assert getattr(spec_method_function, "__self__", None) == plugin_class, f"'{plugin_name}' specification invalid."
    assert inspect.isabstract(plugin_class), f"{plugin_class} is not a plugin superclass."
    ...
    def decorator(decorated_class):
        ...
        assert issubclass(decorated_class, plugin_class), f"{decorated_class} is not a subclass of {plugin_name}"
        assert not inspect.isabstract(decorated_class), f"{decorated_class} is abstract. Plugins may not be abstract."
        ...
        setattr(decorated_class, spec_method_name, classmethod(gds_plugin_implementation(return_decorated_class)))
        return decorated_class
    return decorator
