...
def add_retrieval_arguments(parser: argparse.ArgumentParser, command_name: str, exclude: List[str] = None):
    """
    Adds the arguments that affect how a commands retrieves a message generated by the GDS or F' instance
    """
    # Workaround to using a mutable default argument
    if exclude is None:
        exclude = []
...
def main():
    ...
    if argument_dict["dictionary"] is None:
        ...
        argument_dict["dictionary"] = find_dict(root) # def not needed, handled by StandardPipeline
