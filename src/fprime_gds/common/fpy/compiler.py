...
class CheckCalls(ast.NodeTransformer):
    ...
    def visit_Call(self, node: ast.Call):
        ...
        # only valid calls right now are statements and type instantiations
        if not isinstance(node.func, (FpyStmt, FpyType)):
            node.error = "Invalid syntax"
            return node
    ...
    def visit_Module(self, node: ast.Module):
        for statement in node.body:
            ...
            assert (
                ...
                and isinstance(statement.value.func, (FpyCmd, FpyDirective))
            )
            func = statement.value.func
            # okay, is this a sleep seq dir?
            if isinstance(func, FpyDirective) and func.seq_directive_template.id in [
