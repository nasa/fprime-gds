...
class GenerateExprMacrosAndCmds(Visitor):
    ...
    def visit_AstBreak_AstContinue(
        ...
        # put in a placeholder goto, we will replace it later once we know where the while/for things
        # will end up
        state.directives[node] = [GotoDirective(-1)]
...
class CountNodeDirectives(Visitor):
    ...
    def visit_AstBody(self, node: Union[AstScopedBody, AstBody], state: CompileState):
        count = 0
        if (
...
class GenerateBodyDirectives(Visitor):
    ...
    def visit_AstBreak(self, node: AstBreak, state: CompileState):
        enclosing_loop = state.enclosing_loops[node]
        # go to end of enclosing loop
        end_of_loop_idx = (
            state.start_line_idx[enclosing_loop] + state.node_dir_counts[enclosing_loop]
        )
        state.directives[node] = [GotoDirective(end_of_loop_idx)]

    def visit_AstContinue(self, node: AstContinue, state: CompileState):
        enclosing_loop = state.enclosing_loops[node]
        ...
        if enclosing_loop in state.desugared_for_loops:
            ...
            increment_stmt = enclosing_loop.body.stmts[-1]
            end_of_loop_idx = state.start_line_idx[increment_stmt]
        else:
            # if it's a while loop, go to the beginning
            end_of_loop_idx = state.start_line_idx[enclosing_loop]
        state.directives[node] = [GotoDirective(end_of_loop_idx)]
