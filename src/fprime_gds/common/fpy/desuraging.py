...
class DesugarCheckStmt(Transformer):
    ...
    def visit_AstCheck(self, node: AstCheck, state: CompileState):
        ...
        # check <node.condition> [timeout <node.timeout>] [persist <node.persist>] [every <node.every>]:
        #     <node.body>
        # timeout:
        #     <node.els>
        ...
        # 1 $check_start: Fw.Time = now()
        # 2 $now: Fw.Time = check_start
        # 3 $timeout: Fw.TimeInterval = <node.timeout>
        # 4 $persist: Fw.TimeInterval = <node.persist>
        # 5 $every: Fw.TimeInterval = <node.every>
        # 6 $time_condition_started_succeeding: Fw.Time = Fw.Time(0,0,0,0)
        # 7 $success: bool = False
        # 8 while $now.seconds - $check_start.seconds > $timeout.seconds or 
        #     if <node.condition>:
        #         $time_condition_started_succeeding = now()
        #     if now() - $time_condition_started_succeeding > $persist:
        #         success = True
        #         break
        #     sleep($every.seconds, $every.useconds)
        #     $now = now()
        # 9 if $success:
        #       <node.body>
        #   else:
